...
import { ExternalLink, Trophy, Code2, Terminal, Copy, Check, Globe, BookOpen, X, Sparkles, Server, Zap } from 'lucide-react';
            ...
            <div className="bg-slate-950 border border-slate-800 rounded-lg p-6 max-w-sm w-full shadow-2xl relative overflow-hidden">
              ...
              {isDead ? (
                <div className="text-center mb-6">
                  <h2 className="text-2xl font-black text-rose-500 tracking-tight font-sans mb-1"> CLIENT DEACTIVATED </h2>
                  <p className="text-slate-400 text-xs font-mono"> Your Wolf Bot collided! Length reached: <span className="text-emerald-400 font-bold">{Math.floor(player?.score || 0)}</span>. </p>
                </div>
              ) : (
                <div className="mb-6">
                  <h2 className="text-2xl font-black text-white tracking-tight font-sans mb-2"> WOLF BOT ARENA </h2>
                  <p className="text-slate-400 text-xs font-mono leading-relaxed"> Welcome to the official Studio A7 3D WebGL showcase. Control the cybernetic Wolf Bot, avoid collisions with other units, and collect energy crystals to dominate the arena! </p>
                </div>
              )}
          ...
          <div className="fixed inset-0 flex items-center justify-center pointer-events-auto bg-black/85 backdrop-blur-md z-50 p-4">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-slate-950 border border-slate-800 rounded-lg max-w-2xl w-full h-[520px] shadow-2xl flex flex-col relative overflow-hidden" >
              ...
              <div className="flex bg-slate-900 border-b border-slate-800/60 overflow-x-auto text-[11px]">
                ...
                <button onClick={() => setActiveTab('hosting')} className={`px-4 py-2.5 font-bold transition-all border-r border-slate-800 flex items-center gap-1.5 ${ activeTab === 'hosting' ? 'bg-[#0f172a] text-white border-b-2 border-b-blue-500' : 'text-slate-400 hover:text-slate-200' }`} >
                  <Server size={13} />
                  <span>2. Free Cloud Hosting</span>
                </button>
                <button onClick={() => setActiveTab('seo')} className={`px-4 py-2.5 font-bold transition-all border-r border-slate-800 flex items-center gap-1.5 ${ activeTab === 'seo' ? 'bg-[#0f172a] text-white border-b-2 border-b-blue-500' : 'text-slate-400 hover:text-slate-200' }`} >
                  <Sparkles size={13} />
              ...
              <div className="flex-1 p-5 overflow-y-auto text-xs text-slate-300 leading-relaxed font-sans select-text">
                ...
                {activeTab === 'embed' && (
                  <div className="flex flex-col gap-4">
                    <div>
                      <h3 className="font-bold text-white text-sm font-mono mb-1"> HTML Iframe Embed Code </h3>
                      <p className="text-slate-400 text-[11px]"> Copy this responsive HTML snippet into Webflow, WordPress (Custom HTML), Shopify, or customized HTML pages. It manages pointer locks, fullscreen capabilities, and touch parameters. </p>
                    </div>
                    <div className="bg-slate-900 border border-slate-800 rounded p-3 relative font-mono text-[10px] text-slate-400 whitespace-pre-wrap select-all">
                      {iframeCode}
                      <button onClick={handleCopyCode} className="absolute right-2 top-2 p-1.5 bg-slate-850 hover:bg-slate-800 border border-slate-700/65 rounded text-slate-300 hover:text-white transition-all flex items-center gap-1" >
                    ...
                    <div className="p-3 bg-blue-950/40 border border-blue-900/50 rounded text-[11px] text-blue-300 flex items-start gap-2">
                      ...
                      <p> <strong>Note:</strong> Make sure to replace <code className="bg-blue-900/50 px-1 rounded text-white font-mono">${window.location.origin}</code> with your live hosted cloud production URL once deployed! </p>
                    </div>
                  </div>
                )}
                {/* Tab 2: Free Hosting */}
                {activeTab === 'hosting' && (
                  <div className="flex flex-col gap-4 font-sans">
                    <h3 className="font-bold text-white text-sm font-mono flex items-center gap-1.5"> <Globe size={14} className="text-emerald-400" /> <span>Node.js cloud deployment paths (Free Tiers)</span> </h3>
                    <p className="text-slate-400 text-[11px]"> Because this is a real-time multiplayer multiplayer game using WebSockets, it runs on a custom **Node.js server** instead of static file servers. Here is how to run it for free: </p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-1">
                      <div className="bg-[#111827] p-3.5 rounded border border-slate-800"> <strong className="text-white text-[12px] block mb-1">A. Zeabur (Easiest)</strong> <p className="text-slate-400 text-[11px] leading-relaxed"> 1. Connect your Github or upload the ZIP of this repository.<br /> 2. Zeabur automatically builds and hosts the Node.js server.<br /> 3. Done! Generates robust WebSocket support on a secure domain. </p> </div>
                      <div className="bg-[#111827] p-3.5 rounded border border-slate-800"> <strong className="text-white text-[12px] block mb-1">B. Render (Stable Free Tier)</strong> <p className="text-slate-400 text-[11px] leading-relaxed"> 1. Setup a free account on Render, click **New Web Service**.<br /> 2. Set Build command: <code className="text-slate-200 font-mono text-[9px] bg-slate-800/85 px-1 rounded">npm run build</code>.<br /> 3. Set Start command: <code className="text-slate-200 font-mono text-[9px] bg-slate-800/85 px-1 rounded">npm run start</code>.<br /> 4. Bind environment: <code className="text-slate-200 font-mono text-[9px] bg-slate-800/85 px-1 rounded">PORT=3000</code>. </p> </div>
                    </div>
                    <p className="text-[10px] text-slate-500 font-mono"> Check `/INSTALL_INSTRUCTIONS.md` at the project root for long-term production and VPS configuration options! </p>
                  </div>
                )}
                {/* Tab 3: SEO */}
                {activeTab === 'seo' && (
                  <div className="flex flex-col gap-4">
                    <h3 className="font-bold text-white text-sm font-mono flex items-center gap-1.5"> <Sparkles size={14} className="text-yellow-400" /> <span>Drive organic SEO value for Studio A7 Agency</span> </h3>
                    <p className="text-slate-400 text-[11px]"> How to turn this interactive cyber-wolf game into a powerful backlinks and domain ranking device: </p>
                    <div className="flex flex-col gap-3">
                      <div className="bg-slate-900 border border-slate-800 p-3.5 rounded"> <span className="text-emerald-400 font-bold block mb-1 text-[11px]">1. BOOST DWELL TIME (ORGANIC CORE VIRTUALS)</span> <span className="text-slate-400 text-[11px]"> Placing an interactive, fast 3D WebGL game on a landing page keeps visitors engaged for minutes. Google calculates dwell time (Time-on-Page) as a direct organic authority ranking metric. </span> </div>
                      <div className="bg-slate-900 border border-slate-800 p-3.5 rounded"> <span className="text-emerald-400 font-bold block mb-1 text-[11px]">2. WIDGET VIRALITY BACKLINKS</span> <span className="text-slate-400 text-[11px]"> Allow your clients or guest bloggers to embed this game widget, and hardcode a backlink attribute in the footer pointing back to your prime agency landing page (e.g., <code className="text-slate-300 font-mono">Designed by Studio A7 SEO Agency</code>) to automatically accumulate backlink juice. </span> </div>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="px-4 py-3.5 border-t border-slate-800/80 bg-slate-900/60 flex items-center justify-between text-[11px] text-slate-500">
                ...
                <button onClick={() => setShowEmbedModal(false)} className="px-4 py-1.5 bg-slate-850 hover:bg-slate-800 border border-slate-705/90 rounded text-slate-300 hover:text-white font-bold transition-all" > Done </button>
              </div>
            </motion.div>
          </div>
        )}
