          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 flex items-center justify-center pointer-events-auto bg-[#04060b]/80 backdrop-blur-sm z-50 p-4">
            <div className="bg-slate-950 border border-slate-800 rounded-lg p-6 max-w-sm w-full shadow-2xl relative overflow-hidden">
              ...
              {isDead ? (
                ...
                <div className="mb-6">
                  {/* ZMIANA 1: tytuł i opis po polsku */}
                  <h2 className="text-2xl font-black text-white tracking-tight font-sans mb-2">WOLF BOT ARENA</h2>
                  <p className="text-slate-400 text-xs font-mono leading-relaxed">
                    Wieloosobowa gra zręcznościowa (3D WebGL Multiplayer Online Game). Poprowadź swojego cybernetycznego Wilka SEO do galaktycznej dominacji! Unikaj kolizji z innymi graczami i zbieraj Fotony w kolorze Twojego wilka, by poszerzyć swoją ekspansję na cały świat SEO Universe.
                  </p>
                </div>
              )}
              ...
              <button onClick={joinGame} className="w-full py-3 bg-[#e2e8f0]/95 hover:bg-white text-slate-950 font-bold rounded text-sm transition-all shadow-lg active:scale-95 flex items-center justify-center gap-2 cursor-pointer">
                <Zap size={14} />
                {/* ZMIANA 2: etykiety przycisków po polsku */}
              ...
              <div className="flex-1 p-5 overflow-y-auto text-xs text-slate-300 leading-relaxed font-sans select-text">
                ...
                {activeTab === 'hosting' && (
                  <div className="flex flex-col gap-4 font-sans">
                    ...
                    <p className="text-slate-400 text-[11px]">Because this is a real-time multiplayer game using WebSockets, it runs on a custom **Node.js server** instead of static file servers. Here is how to run it for free:</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-1">
                      <div className="bg-[#111827] p-3.5 rounded border border-slate-800">
                        ...
                        <p className="text-slate-400 text-[11px] leading-relaxed">1. Connect your Github or upload the ZIP of this repository.<br />2. Zeabur automatically builds and hosts the Node.js server.<br />3. Done! Generates robust WebSocket support on a secure domain.</p>
                      </div>
                      <div className="bg-[#111827] p-3.5 rounded border border-slate-800">
                        <strong className="text-white text-[12px] block mb-1">B. Render (Stable Free Tier)</strong>
                        <p className="text-slate-400 text-[11px] leading-relaxed">1. Setup a free account on Render, click **New Web Service**.<br />2. Set Build command: <code className="text-slate-200 font-mono text-[9px] bg-slate-800/85 px-1 rounded">npm run build</code>.<br />3. Set Start command: <code className="text-slate-200 font-mono text-[9px] bg-slate-800/85 px-1 rounded">npm run start</code>.<br />4. Bind environment: <code className="text-slate-200 font-mono text-[9px] bg-slate-800/85 px-1 rounded">PORT=3000</code>.</p>
                      </div>
                    </div>
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-3">
                      <div className="bg-slate-900 border border-slate-800 p-3.5 rounded">
                        ...
                        <span className="text-slate-400 text-[11px]">Placing an interactive, fast 3D WebGL game on a landing page keeps visitors engaged for minutes. Google calculates dwell time (Time-on-Page) as a direct organic authority ranking metric.</span>
                      </div>
                      <div className="bg-slate-900 border border-slate-800 p-3.5 rounded">
                        <span className="text-emerald-400 font-bold block mb-1 text-[11px]">2. WIDGET VIRALITY BACKLINKS</span>
                        <span className="text-slate-400 text-[11px]">Allow your clients or guest bloggers to embed this game widget, and hardcode a backlink attribute in the footer pointing back to your prime agency landing page (e.g., <code className="text-slate-300 font-mono">Designed by Studio A7 SEO Agency</code>) to automatically accumulate backlink juice.</span>
                      </div>
                    </div>
                  </div>
                )}
